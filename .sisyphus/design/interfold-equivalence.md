...
- The folding/compression layer (Nova Nova IVC) targets O(polylog n) verifier cost, while Interfold's wrapper circuits target a different cost model.
...
| Proof compression | Recursive wrapper circuits | Nova Nova IVC compression over BN254+grumpkin |
...
C0, C2a and C2b are now `implemented`: C0 keygen NIZK uses real BFV sigma proofs; the E.1 batched Shamir/RS share-computation relation (`share_computation.rs`) covers both sk and e_sm tracks with low-degree/RS validity checks, coefficient bounds, and foldable public instance commitments. C7 (final decryption aggregation) is now `partial` with Nova C7DecryptAggregationCircuit and C7MerkleStepCircuit implementations. The remaining `partial` entries (C1, C3, C4, C5, C6) have types, working flows, and NIZK infrastructure, though C1, C3, and C5 carry structural gaps (see §C1, §C3, and §C5 below).
