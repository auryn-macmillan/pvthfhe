...
C2a and C2b are now `implemented`: the E.1 batched Shamir/RS share-computation relation (`share_computation.rs`) covers both sk and e_sm tracks with low-degree/RS validity checks, coefficient bounds, and foldable public instance commitments. C7 (final decryption aggregation) remains the largest gap between PVTHFHE's current state and the Interfold guarantee surface. The remaining `partial` entries (C0, C1, C3, C4, C5, C6) have types, working flows, and NIZK infrastructure, though C1, C3, and C5 carry structural gaps (see §C1, §C3, and §C5 below). Closing C7 is the primary architectural work of Batch G in the plan.
