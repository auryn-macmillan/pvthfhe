...
### Symptom
`NovaNova::verify()` returns `NotSatisfied` at the incoming instance `check_relation`. Specifically:
...
- C7_STEP_DATA is set BEFORE `NovaCompressor::new` (so R1CS is generated with actual coefficient data, not zeros)
...
- `crates/pvthfhe-compressor/src/nova/mod.rs` — `verify_steps_c7`
...
PVTHFHE_I_UNDERSTAND_INSECURE_RNG=1 cargo run --release -p pvthfhe-cli \
  --features "nova-compressor,demo-seeded-rng,pipeline-extra-checks" \
