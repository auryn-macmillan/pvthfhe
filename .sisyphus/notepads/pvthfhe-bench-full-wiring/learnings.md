...
- Added five RED-first integration tests in `crates/pvthfhe-cli/tests/e2e_phase_timing.rs` for `nizk_prove`, `keygen`, `pvss_decrypt_prove`, `cyclo_fold`, and the `noir_nova_wrap` marker; the full mock test target now passes all 10 timing tests.
- `crates/pvthfhe-cli/src/bin/pvthfhe_e2e.rs` now records aggregate keygen time, per-dealer NIZK prove timings, merged `cyclo_fold` time, and a marker-only `noir_nova_wrap` phase; `run_lattice_pvss` artifacts are also copied into `phases.pvss_decrypt_prove`.
...
## 2026-05-07 Probe P0' — Nova isolated memory investigation
- `crates/pvthfhe-compressor/tests/nova_isolated_mem.rs` currently samples `/proc/self/statm` every 100ms around `NovaCompressor::new(1)`, `prove`, and `verify`; the test compiles/runs and is being held RED intentionally by a terminal panic after reporting the observed peak RSS.
- `crates/pvthfhe-compressor/src/nova/mod.rs` now emits `tracing::info!` RSS markers after parameter serialization, at `deserialize_params` start / post-prover-deserialize / post-verifier-deserialize, after `Nova::init`, after each `prove_step`, and after IVC proof serialization.
- `crates/pvthfhe-compressor/examples/nova_isolated.rs` plus `.sisyphus/scripts/run-nova-isolated.sh` produced runnable evidence under `.sisyphus/evidence/bench-comparison-mem/p0p/`; the captured run showed RSS rising from ~93 MiB after `new()` to ~223 MiB after `prove()`, while `/usr/bin/time -v` reported max RSS 244112 KiB.
- Post-change verification succeeded for `cargo build --release --example nova_isolated -p pvthfhe-compressor`, `cargo test -p pvthfhe-compressor --test nova_roundtrip`, `--test spec_addendum_present`, and `--test trait_object`; the full `cargo test -p pvthfhe-compressor` now fails only because the new RED probe test is intentionally failing.
