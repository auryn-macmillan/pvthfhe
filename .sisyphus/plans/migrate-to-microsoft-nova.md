# Migrate Nova `folding-schemes` → Microsoft Nova `nova-snark`

**Status**: PLAN
**Goal**: Replace Nova's `folding-schemes` with Microsoft's `nova-snark` to fix the `pp_hash` serialization bug causing `IVCVerificationFail` in C4/C5 verification.
...
**Why not the simpler pre-seed fix**: The pre-seed approach (`set_dkg_agg_data` before `NovaCompressor::new()`) was tested with matching data shapes (n×n for C4, 1×n for C5) and still fails with `IVCVerificationFail`. The root cause is a `pp_hash` consistency bug in Nova's `VerifierParams` serialize/deserialize round-trip (line 416 of Nova's `mod.rs`), not a constraint-count mismatch. Pre-seeding fixes the R1CS shape but not the pp_hash divergence.
...
| `nova/mod.rs` (~900 lines) | Rewrite `NovaCompressor` wrapper using `RecursiveSNARK` API |
...
| `NovaNova::preprocess` | `PublicParams::setup` |
...
- [x] Rewrite NovaCompressor for arecibo (prove_steps/verify_steps compile)
