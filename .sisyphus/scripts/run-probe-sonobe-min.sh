#!/usr/bin/env bash
# Run the minimal Sonobe-only probe with a strict virtual-memory cap and
# sample VmRSS from /proc/<pid>/status every 5s.
# Detached usage:
#   setsid nohup .sisyphus/scripts/run-probe-sonobe-min.sh </dev/null >.../launcher.out 2>&1 & disown
set -uo pipefail

ROOT="/home/dev/pvthfhe"
EVIDENCE="$ROOT/.sisyphus/evidence/bench-comparison-mem/probe-sonobe-min"
