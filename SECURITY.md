...
>
> See [SECURITY-ADVISORY-001.md](docs/archive/SECURITY-ADVISORY-001.md) (archived, resolved), [WARNING.md](WARNING.md), and [docs/OPEN-PROBLEM-BLOCKERS.md](docs/OPEN-PROBLEM-BLOCKERS.md) for details.
...
- **LatticeFold+ Folding**: Per-channel native folding over RNS primes (q0, q1, q2 at ~58 bits each from fhe.rs) via `pvthfhe-rings` + `pvthfhe-cyclo`. Each channel gets its own cyclotomic ring `R_{q_l} = Z_{q_l}[X]/(X^N+1)` with dedicated NTT context; `mod q_l` and `mod X^N+1` are free (ring equality). Track A (Nova SNARK BN254+Grumpkin) removed per P4 deprecation. The folding chain provides soundness guarantees through transparent lattice verification — no Groth16 trusted ceremony required, no elliptic curve assumptions. Production ring N=8192; fast-test via `--features fast-ring-n256` (N=256 commitment ring).
- **NIZK proofs**: Ajtaï D2 sigma + BFV sigma with k-round parallel repetition. LatticeFold+ algebraic range proof (per `pvthfhe-cyclo/src/range_check.rs`) enforces norm bounds, eliminating the GRECO quotient-witness machinery previously needed for BFV encryption NIZK soundness. M7 fix (2026-06-05): zero-witness rejection via Ajtai commitment all-zeros check.
