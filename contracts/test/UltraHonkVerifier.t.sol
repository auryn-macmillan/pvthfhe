...
contract UltraHonkVerifierTest is Test {
    ...
    function setUp() public {
        ...
        proof = vm.readFileBinary(
            string.concat(vm.projectRoot(), "/../circuits/ivc_state_commitment/target/proof")
        );
        publicInputs = _decodePublicInputs(
            vm.readFileBinary(
                string.concat(vm.projectRoot(), "/../circuits/ivc_state_commitment/target/public_inputs")
            )
        );
    }
