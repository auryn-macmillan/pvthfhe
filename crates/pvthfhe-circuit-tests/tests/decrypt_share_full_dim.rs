...
#[test]
fn decrypt_share_full_dimension_harness_runs_canonical_bb_flow(
) -> Result<(), Box<dyn std::error::Error>> {
    if !tool_in_path("nargo") || !tool_in_path("bb") {
        eprintln!(
            "skipping decrypt_share full-dimension test because nargo or bb is unavailable in PATH"
        );
        return Ok(());
    }

    let prover_toml = repo_root().join("circuits/decrypt_share/Prover.toml");
    let nargo_artifacts = nargo::execute("decrypt_share", &prover_toml)?;
    assert!(nargo_artifacts.witness_path.is_file());
    assert!(nargo_artifacts.bytecode_path.is_file());

    let bb_artifacts = bb::write_vk_prove_verify("decrypt_share", "ultra_honk")?;
    ...
    assert_eq!(
        ...
        "decrypt_share public input count should match the full-dimension compact statement"
    );
