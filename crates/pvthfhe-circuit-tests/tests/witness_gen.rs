...
use pvthfhe_circuit_tests::witness_gen::{
    generate_decrypt_share_witness, noir_bind_8_with_domain, noir_vector_hash, B_E, DECRYPT_SHARE_N,
};
...
fn generated_decrypt_share_witness_matches_circuit_invariants() {
    ...
    let sk_fr: Vec<Fr> = witness
        .sk_i
        .iter()
        .map(|v| v.parse::<Fr>().unwrap())
        .collect();
    let c1_fr: Vec<Fr> = witness
        .c1
        .iter()
        .map(|v| v.parse::<Fr>().unwrap())
        .collect();
    let d_i_fr: Vec<Fr> = witness
        .d_i
        .iter()
        .map(|v| v.parse::<Fr>().unwrap())
        .collect();
    ...
    assert_eq!(
        witness.pk_i_hash,
        noir_vector_hash(&sk_fr, Fr::from(1u64)).to_string()
    );
    assert_eq!(
        witness.c1_hash,
        noir_vector_hash(&c1_fr, Fr::from(1u64)).to_string()
    );
    assert_eq!(
        witness.d_i_hash,
        noir_vector_hash(&d_i_fr, Fr::from(1u64)).to_string()
    );
