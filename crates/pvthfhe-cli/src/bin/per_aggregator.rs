...
fn main() -> anyhow::Result<()> {
    ...
    #[cfg(not(feature = "fast-ring-n256"))]
    {
        let t4 = Instant::now();
        let mut driver =
            pvthfhe_cyclo::channel_fold::production_driver().context("per-channel driver")?;
        let chan_count = driver.channel_count();
        let degree = driver.ring(0).degree();

        for instance in &fold_instances {
            let commitment =
                ajtai::decode_commitment(instance.ajtai_commitment_bytes.as_slice(), 13)
                    .context("decode")?;
            let per_ch: Vec<pvthfhe_rings::RqPoly> = (0..chan_count)
                .map(|ch| {
            ...
            let witnesses: Vec<_> = (0..chan_count)
                ...
                .collect();
            driver
                .fold_one(&per_ch, &witnesses)
                .context("per-channel fold")?;
        }
        let pc_ms = t4.elapsed().as_secs_f64() * 1000.0;
        println!(
            "  per_channel:   {:>8.1} ms  ({chan_count} channels)",
            pc_ms
        );
        for ch in 0..chan_count {
            println!(
                "    channel {ch}: mod={} fold_count={}",
