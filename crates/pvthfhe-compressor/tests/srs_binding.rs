...
fn srs_is_derived_from_epoch_hash_not_seed() {
    ...
    let comp_a = NovaCompressor::<DkgAggregationStepCircuit<Fr>>::new(epoch_a, 4)
        .expect("construct compressor with epoch_a");
    let comp_b = NovaCompressor::<DkgAggregationStepCircuit<Fr>>::new(epoch_b, 4)
    ...
    let comp_a2 = NovaCompressor::<DkgAggregationStepCircuit<Fr>>::new(epoch_a, 4)
