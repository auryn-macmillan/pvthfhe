...
fn srs_hash_method_exists_and_is_deterministic() {
    ...
    let compressor2 = SonobeCompressor::<ToyStepCircuit<Fr>>::new(epoch, 4)
        .expect("construct second compressor");
    assert_eq!(hash, compressor2.srs_hash(), "srsHash must be deterministic for same epoch");
    ...
    assert_ne!(hash, compressor3.srs_hash(), "srsHash must differ for different epochs");
