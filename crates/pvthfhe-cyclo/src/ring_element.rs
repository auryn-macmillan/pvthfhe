//! Generic ring element arithmetic over R = Z_F[X]/(X^N + 1) for any prime
//! field `F`.
//!
//! This is the canonical home of the generic-field ring helper used by the
//! CCS verifier equation and folding-witness validation (moved from
//! `pvthfhe-aggregator::folding::ring_element` in Phase 3.4 of the
//! 2026-07-24 repo refactor; the aggregator re-exports it unchanged).
//!
//! Unlike [`crate::ring`] — which is fixed to the commitment ring
//! `Z_{q_commit}[X]/(X^256+1)` with NTT acceleration — this type is generic
//! over any `ark_ff::PrimeField` and uses schoolbook convolution. On the
//! shared domain `F = F_{q_commit}` the two implementations agree
//! coefficient-by-coefficient (pinned by
