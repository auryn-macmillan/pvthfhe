...
//! # Safety opt-in
//!
//! Every entry point of the public [`MockBackend`] checks that the environment
//! variable `PVTHFHE_I_UNDERSTAND_THIS_IS_A_MOCK` is set to `"1"` at runtime.
...
//!
//! # Always-compiled internals
//!
//! The module itself is always compiled: [`parse_params`] (the canonical
//! `[rlwe]` TOML parser) is used by [`crate::fhers`] regardless of features.
//! [`MockBackendInner`] and the public [`MockBackend`] wrapper exist only when
//! `production-profile` is off. (Merged from the former `mock.rs` +
//! `mock_impl.rs` pair in Phase 3.5 of the 2026-07-24 repo refactor — the
//! mock backend is one concept.)
