//! Integration tests gating the production-profile feature invariants.
...
fn production_profile_exists_and_excludes_mock() {
    ...
    let prod_profile = features_section
        .lines()
        .find(|line| line.trim_start().starts_with("production-profile ="));

    assert!(
        prod_profile.is_some(),
        "production-profile feature must exist"
    );
