...
% LatticeFold+ lattice-native folding (Track B, sole backend); Track A (Nova BN254+Grumpkin) REMOVED
...
single canonical DKG path (Track A removed, per P4 deprecation) with \texttt{FoldTrackKind::Sk} and \texttt{FoldTrackKind::ESm} share tracks;
...
cost from $O(n)$ to $O(\text{polylog}\, n)$.  The PVTHFHE protocol uses a
single folding backend at this layer:

\begin{description}
\item[Track A (Concrete, \S\ref{sec:p2-tracka}):]
  Sonobe Nova IVC with CycloFoldStepCircuit (REMOVED per P4 deprecation, 2026-06-07).
  Formerly the concrete path; all code, dependencies, and tests deleted.
  See \texttt{.sisyphus/plans/p4-track-a-deprecation.md}.

\item[Track B (Target, \S\ref{sec:p2-trackb}):]
  LatticeFold+ over RLWE accumulates lattice NIZK witnesses by algebraic
  folding.  \textbf{Now the sole IMPLEMENTED backend} — Cyclo crate provides
  fold\_one\_deterministic, verify\_fold, check\_satisfiability, Ajtai commitments,
  and CCS encoding with Fiat-Shamir challenge derivation (challenge\_v2 with
  params\_digest binding).  15 tests pass including forgery resistance.
\end{description}
...
% ===================================================================
\subsection{Track A: Sonobe Nova IVC (Concrete) \textbf{[REMOVED — P4 DEPRECATION, 2026-06-07]}}
\label{sec:p2-tracka}

Track~A used Sonobe Nova IVC~\cite{nova2022} with a custom
\texttt{CycloFoldStepCircuit} operating on three $\mathbb{F}_r$ scalars
encoding SHA-256 accumulator state.  This track was fully implemented and
benchmarked but has been removed from the codebase per the P4 Track~A
deprecation plan (\texttt{.sisyphus/plans/p4-track-a-deprecation.md}).
All Nova SNARK dependencies (nova-snark, ark-grumpkin, BN254/Grumpkin curve
cycle, bellpepper-core, ark-r1cs-std) and associated code (\texttt{src/nova/},
\texttt{src/micronova/}, $\sim$4,800 lines) have been deleted.  Track~B
(LatticeFold+) is now the sole folding backend.  The theorems below are
retained for provenance but carry historical status only.

\paragraph{C7 In-Circuit Commitment Verification (G2 Full).}
The C7 decryption aggregation circuit (\texttt{C7DecryptAggregationCircuit})
folds per-participant Lagrange recombination evaluations.  Following the
G2 full upgrade (plan \texttt{g2-full-in-circuit-poseidon.md}, status DONE),
the circuit now verifies three properties entirely in R1CS constraints
without trusting any external witness data:
\begin{enumerate}
  \item \textbf{Commitment Opening.} The 8192 coefficient witnesses are
    hashed by a Poseidon sponge in-circuit ($\sim$614K constraints) and
    enforced equal to the external commitment hash (ext.2).  This replaces
    the previous off-circuit Merkle-tree verification that only checked a
    single leaf.
  \item \textbf{Challenge-Point Correctness.} The evaluation powers
    $r^0, r^1, \dots, r^{8191}$ are constrained consecutively
    ($r^0 = 1$, $r^{j+1} = r^j \cdot r$), adding 8,191
    multiply-enforce-equal constraints per step.  The challenge $r$ itself
    is derived in-circuit as $\operatorname{Poseidon}(\mathit{coeff\_commitment}
    \;\|\; \mathit{dkg\_root\_hash})$.
  \item \textbf{Complete Trust Closure.} Together, these constraints ensure
    that the aggregation result binds to the committed polynomial
    coefficients with no trust gap: a malicious prover cannot substitute
    coefficients, choose a favorable challenge point, or provide mismatched
    powers.
\end{enumerate}
The G2 full circuit uses $\sim$639K constraints per step (was $\sim$8K for
the bare evaluation); at $t = 64$ the total is $\sim$41M constraints,
well within Nova's practical range.  The per-aggregator C7 phase
completes in 2.8~s at $n=10, t=4$ in the end-to-end demo.
...
VERDICT: PROVED-WITH-CITATION --- VERIFIED: norm enforcement implemented via CycloFoldStepCircuit state\_len=7 (G7b: $z_s$\_sq\_acc, $z_e$\_sq\_acc norm accumulators). Lattice-commitment replacement remains open).
\end{theorem}
...
% ===================================================================
\subsection{Track B: LatticeFold+ over RLWE (Target) \textbf{[SOLE BACKEND]}}
\label{sec:p2-trackb}

The implemented P2 path uses \textbf{LatticeFold+} accumulation that folds RLWE ciphertext
witnesses directly via algebraic relations, with a CCS encoding of the
Cyclo decryption relation~\cite{cyclo2025,latticefold2024}.  This is the
sole folding backend after Track~A removal (P4).  The Cyclo crate
(\texttt{pvthfhe-cyclo}) provides: \texttt{fold\_one\_deterministic},
\texttt{verify\_fold}, \texttt{check\_satisfiability} (real CCS relation
$M \cdot z \odot z = 0$ over BN254 $\mathbb{F}_r$), Ajtai lattice
commitments ($Q_{\text{commit}} = 2^{49}$, 13 ring elements $\times$ 256
coeffs), and Fiat-Shamir challenge derivation (\texttt{challenge\_v2} with
\texttt{params\_digest} binding).  All 15 Cyclo tests pass including forgery
resistance.
...
\item[Track B (On-chain UltraHonk, \S\ref{sec:p3-trackb}):]
  UltraHonk + MicroNova on-chain verification of the Sonobe Nova terminal
  accumulator.  \textbf{Status: VERIFIED locally.}  A real UltraHonk proof
...
rushing adversaries. We have implemented Reed--Solomon parity-check proofs (\texttt{parity.rs}) that validate all $n$ shares are evaluations of a single degree-$t$ polynomial. Combined with Nova-folded DKG verification, this achieves $O(1)$ per-recipient DKG verification, a concrete step toward robust secret sharing.
