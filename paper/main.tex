...
verification cost. The current implementation uses a trusted-signer surrogate for P3,
with full cryptographic verification identified as future work.
...
\label{thm:p2-t1}
Honest P1 proofs fold into an accepting accumulator under the frozen verifier equation (simulated in the current SHA-256 hash-chain surrogate).
\end{theorem}

\begin{theorem}[P2 Knowledge Soundness]
\label{thm:p2-t2}
Under the proposed LatticeFold+ architecture, a depth-$d$ accepting fold tree yields valid RLWE witnesses except with $(1/3)^d$ plus SHA-256 binding failure probability.
\end{theorem}

\begin{theorem}[P2 ZK Preservation]
\label{thm:p2-t3}
The proposed folding scheme preserves the projected SLAP core zero-knowledge view under ROM + HVZK assumptions.
\end{theorem}

\begin{theorem}[P2 Accumulator Binding]
\label{thm:p2-t4}
The accumulator commitment is designed to be binding under RingSIS/M-SIS; the current surrogate uses SHA-256 binding.
\end{theorem}

\begin{theorem}[P2 On-chain Compatibility]
\label{thm:p2-t5}
The final accumulated proof targets Solidity/Yul verification within bounded gas and proof size (current benchmarks use the P3 surrogate).
\end{theorem}
...
\label{thm:p3-t1}
Any on-chain acceptance of the P3 verifier implies valid ECDSA authorization by the TRUSTED\_SIGNER; cryptographic verification of the P2 terminal accumulator is deferred.
\end{theorem}
...
\label{thm:p3-t5}
A valid P3 submission either finalizes on-chain or aborts; automated blame predicates for P3 are deferred.
\end{theorem}
...
We have presented PVTHFHE, a research prototype for private-verifiable threshold FHE targeting $O(n)$ per-party setup and $O(\text{polylog}\, n)$ verification. We provide a complete four-layer construction, noting that P2 and P3 currently use cryptographic surrogates. Sub-protocols have formal security proof drafts and the implementation is validated via benchmarks.
