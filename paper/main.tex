...
% LatticeFold+ per-channel native folding (pvthfhe-rings, pvthfhe-cyclo); Track A removed June 2026
% committed-smudge decryption; G2 full in-circuit Poseidon commitment verification
% Per-channel RNS rings (q0/q1/q2 from fhe.rs, N=8192) eliminate GRECO overhead; G-N8 RESOLVING
% UltraHonk decider-wrapper Solidity verifier (target: N=65536, 28 G1 points)
...
The Cyclo crate (\texttt{pvthfhe-cyclo}) and the per-channel rings crate (\texttt{pvthfhe-rings}) provide:
\texttt{fold\_one\_deterministic}, \texttt{verify\_fold},
\texttt{check\_satisfiability} (real CCS relation $M \cdot z \odot z = 0$
over BN254 $\mathbb{F}_r$), per-channel Ajtai lattice commitments (three RNS
primes $q_0,q_1,q_2$ at $\approx 2^{58}$ each, $N=8192$ cyclotomic rings),
and Fiat-Shamir challenge derivation (\texttt{challenge\_v2} with
\texttt{params\_digest} binding).  All 32~Cyclo tests pass including forgery
resistance.  In progress: native per-channel folding (branch
\texttt{feat/native-per-channel}) replaces the single-ring N=256 commitment
path with independent LatticeFold+ tracks per RNS prime, eliminating the
Noir circuit ring-dimension ceiling (G-N8).

(The heterogeneous IVC prototype previously evaluated here — a MicroNova
compressor with LatticeFoldTreeCircuitFamily — was retired with the
Track~A Nova dependency; see git history for that experiment.)
...
\textbf{Status: ASPIRATIONAL} (gas figure projected from earlier
MicroNova-based estimates; not yet measured for the implemented
Ajtai-commitment path).
\end{theorem}
...
\subsection{UltraHonk On-Chain Verification}
\label{sec:p3-ultrahonk}

The P3 path replaces the former trusted-signer surrogate with a
full on-chain cryptographic verifier.  \textbf{Implementation note
(2026-07):} the originally projected MicroNova compression stage was
retired together with the Track~A Nova dependency; the implemented
stack is:

\begin{enumerate}
\item \textbf{LatticeFold+} folds the recursive proof tree into a single
  constant-size accumulator committed via lattice-native Ajtai
  commitments (no trusted setup, no elliptic-curve assumptions);
\item \textbf{UltraHonk} verifies the Ajtai commitment and IVC state
  binding on-chain (Noir $\to$ Barretenberg $\to$ Solidity
  \texttt{HonkVerifier}) via EIP-196/197 BN254 pairing precompiles.
\end{enumerate}
...
\begin{theorem}[P3-T2: Binding Preserves Soundness]
\label{thm:p3-t2}
The Ajtai-commitment $\to$ UltraHonk binding preserves the LatticeFold+
...
\label{thm:p3-t3}
UltraHonk and the Ajtai commitment scheme both require no trusted setup.
\textbf{Status: SKELETON}.
\end{theorem}

\begin{theorem}[P3-T4: Gas Bound]
\label{thm:p3-t4}
Projected UltraHonk gas: $39{,}687$ (earlier MicroNova-based projection;
...
(a) completed on-chain verification of the full LatticeFold+ terminal
accumulator (open problem P4 below), and
(b) UltraHonk verifier contract deployment on a public testnet.
...
UltraHonk path verifies the Ajtai commitment and IVC state binding;
direct on-chain verification of the full LatticeFold+ accumulator is
projected for future work.
...
(The audit remediation history is documented in the artifact appendix,
\texttt{paper/artifact-appendix.md}.)
